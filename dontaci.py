...
while 1:
  ...
  ╚═════╝  ╚═════╝ ╚═╝  ╚═══╝   ╚═╝   ╚═╝  ╚═╝ ╚═════╝╚═╝    ╚══════╝ ╚═════╝╚═╝  ╚═╝╚═╝╚═╝        ╚═╝   
    ...
    Call: {}              {}insta {}@dot.wq\n  
    """.format(Fore.LIGHTMAGENTA_EX, len(servisler_sms), len(servisler_call), Style.RESET_ALL, Fore.LIGHTRED_EX))
    try:
        menu = int(input(Fore.LIGHTMAGENTA_EX + "   SMS Gönder 1: [1] / Ara 2: [2] ] / Çıkış 3: [3]" + Fore.LIGHTYELLOW_EX + "\n\n           > Select Mode Number: >> "))
    except ValueError:
        system("cls||clear")
                ...
                if callable(attribute_value):
                    if attribute.startswith('__') == False:
                        if call.adet == kere:
                            break
                        exec("call."+attribute+"()")
        print(Fore.GREEN+ "\nArama gönderme işlemi bitmiştir..")
